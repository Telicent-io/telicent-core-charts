...
class Console:
    ...
    @classmethod
    def print(self, message: str, colour: Colours=Colours.NC, indent: int=0, prefix: str='', end: str='\n') -> None:
        ...
        output = prefix + f'{prefix}'.join([l for l in lines])
        print(textwrap.indent(f'{colour}{output}{Colours.NC}', indent_str), end=end)
...
def process_chart(chart_name: str, chart_path: str) -> bool:
    ...
    if not os.path.exists(values_file):
        chart_errors['missing_values_file'].append(chart_name)
    ...
    if not os.path.exists(readme_config_file):
        chart_errors['missing_readme_config_file'].append(chart_name)
    ...
    if process.returncode == 0:
        ...
        Console.print(output.decode(), indent=4, prefix='| ')
        ...
        if charts:
            ...
            if isinstance(charts, dict):
                for chart, errors in sorted(charts.items()):
                    ...
                    Console.print(', '.join([e[e.rindex(':')+1:].strip() for e in errors]))
