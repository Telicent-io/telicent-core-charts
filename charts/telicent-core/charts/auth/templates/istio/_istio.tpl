...
b.) when installed through a sub chart, where release name equals 'auth' (chart name) the
    release name will not be included.
*/}}
{{- define "auth.calServiceAccount" -}}
{{- $envVal := index . 0 -}}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "auth.calServiceAccount" (list . .Values.istio.ingress.serviceAccountName) ) }}
{{- end }}
{{- end }}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "auth.calServiceAccount" (list . .Values.istio.userPreferences.serviceAccountName) ) }}
{{- end }}
{{- end }}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "auth.calServiceAccount" (list . .Values.istio.graph.serviceAccountName) ) }}
{{- end }}
{{- end }}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "auth.calServiceAccount" (list . .Values.istio.search.serviceAccountName) ) }}
{{- end }}
{{- end }}
{{- end }}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "auth.calServiceAccount" (list . .Values.istio.paperbackWriter.serviceAccountName) ) }}
{{- end }}
{{- end }}
{{- end }}
