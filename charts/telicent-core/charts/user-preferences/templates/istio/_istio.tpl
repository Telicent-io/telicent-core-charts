...
b.) when installed through a sub chart, where release name equals 'user-preferences' (chart name) the
    release name will not be included.
*/}}
{{- define "user-preferences.calServiceAccount" -}}
{{- $envVal := index . 0 -}}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "user-preferences.calServiceAccount" (list . .Values.istio.ingress.serviceAccountName) ) }}
{{- end }}
{{- end }}
