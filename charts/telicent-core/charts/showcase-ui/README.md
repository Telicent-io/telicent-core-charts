# Telicent Package for Showcase UI

Telicent Showcase UI is an application for querying data in Telicent CORE.

## Introduction

This chart bootstraps Telicent Showcase UI deployment on a [Kubernetes](https://kubernetes.io) cluster using
...
```console
helm install my-release ./charts/telicent-core/charts/showcase-ui
```
...
.dev/readme-generator-for-helm --config=charts/telicent-core/readme.config \
 --values=charts/telicent-core/charts/showcase-ui/values.yaml \
 --readme=charts/telicent-core/charts/showcase-ui/README.md \
 --schema=charts/telicent-core/charts/showcase-ui/values.schema.json
```
...
Contains parameters specific to the Showcase User Interface.


| Name                         | Description                                                                                                        | Value                      |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------- |
| `ui.searchUiDeployed`        | If set to true, *Search UI* links will be available within *Showcase UI*                                              | `true`                     |
| `ui.userPortalUiDeployed`    | If set to true, *User Portal UI* links will be available within *Showcase UI*                                         | `true`                     |
| `ui.dataCatalogUiDeployed`   | If set to true, *Data Catalog UI* links will be available within *Showcase UI*                                        | `true`                     |
...
| Name             | Description                                   | Value                           |
| ---------------- | --------------------------------------------- | ------------------------------- |
| `oauth.clientId` | The OAuth client id to be used by *Showcase UI*  | `telicent-showcase-ui`             |
| `oauth.scopes`   | List of OAuth scopes to be used by *Showcase UI* | `openid profile offline_access` |
...
| Name                   | Description                                                     | Value |
| ---------------------- | --------------------------------------------------------------- | ----- |
| `replicas`             | Number of *Showcase UI* replicas to deploy                         | `1`   |
| `revisionHistoryLimit` | Number of controller revisions to keep                          | `5`   |
| `annotations`          | Add extra annotations to the deployment object                  | `{}`  |
| `podLabels`            | Add extra labels to the *Showcase UI* pod                          | `{}`  |
| `podAnnotations`       | Add extra annotations to the *Showcase UI* pod                     | `{}`  |
| `extraEnvVars`         | Array with extra environment variables to add to *Showcase UI* pod | `[]`  |
...
| Name                | Description                                                               | Value                     |
| ------------------- | ------------------------------------------------------------------------- | ------------------------- |
| `image.registry`    | *Showcase UI* image registry                                                 | `quay.io`                 |
| `image.repository`  | *Showcase UI* image name                                                     | `telicent/telicent-showcase` |
| `image.tag`         | *Showcase UI* image tag. If not set, a tag is generated using the appVersion | `""`                      |
| `image.pullPolicy`  | *Showcase UI* image pull policy                                              | `IfNotPresent`            |
...
| Name        | Description                         | Value |
| ----------- | ----------------------------------- | ----- |
| `resources` | Resources for *Showcase UI* containers | `{}`  |
...
| Name           | Description                                                                 | Value       |
| -------------- | --------------------------------------------------------------------------- | ----------- |
| `service.name` | *Showcase UI* service name. If not set, a name is generated using the fullname | `""`        |
| `service.port` | *Showcase UI* service port                                                     | `8080`      |
| `service.type` | *Showcase UI* service type                                                     | `ClusterIP` |

### Host(s) Parameters - Contains host information for applications deployed via *telicent-core* chart.

*Showcase UI* interacts directly with other Telicent Applications using their default service/serviceAccount and port.
