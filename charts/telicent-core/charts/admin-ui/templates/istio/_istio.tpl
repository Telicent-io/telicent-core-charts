...
b.) when installed through a sub chart, where release name equals 'admin-ui' (chart name) the
    release name will not be included.
*/}}
{{- define "admin-ui.calServiceAccount" -}}
{{- $envVal := index . 0 -}}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "admin-ui.calServiceAccount" (list . .Values.istio.ingress.serviceAccountName) ) }}
{{- end }}
{{- end }}
