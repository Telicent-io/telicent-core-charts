...
b.) when installed through a sub chart, where release name equals 'traefik-proxy' (chart name) the
    release name will not be included.
*/}}
{{- define "traefik-proxy.calServiceAccount" -}}
{{- $envVal := index . 0 -}}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "traefik-proxy.calServiceAccount" (list . .Values.istio.ingress.serviceAccountName) ) }}
{{- end }}
{{- end }}
