...
b.) when installed through a sub chart, where release name equals 'graph-ui' (chart name) the
    release name will not be included.
*/}}
{{- define "graph-ui.calServiceAccount" -}}
{{- $envVal := index . 0 -}}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "graph-ui.calServiceAccount" (list . .Values.istio.ingress.serviceAccountName) ) }}
{{- end }}
{{- end }}
