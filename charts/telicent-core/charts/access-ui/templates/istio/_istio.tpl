...
b.) when installed through a sub chart, where release name equals 'access-ui' (chart name) the
    release name will not be included.
*/}}
{{- define "access-ui.calServiceAccount" -}}
{{- $envVal := index . 0 -}}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "access-ui.calServiceAccount" (list . .Values.istio.ingress.serviceAccountName) ) }}
{{- end }}
{{- end }}
