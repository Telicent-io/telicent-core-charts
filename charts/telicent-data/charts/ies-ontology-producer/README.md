...
A Kafka producer component that operates as part of the Telicent Data parent chart, designed to publish IES (Information Exchange Standard) ontology data to Kafka topics for semantic data processing.

## Overview

This chart deploys the IES Ontology Producer, which processes and publishes IES ontology definitions and semantic structures to configured Kafka topics within the Telicent data ecosystem. The producer enables semantic data processing and knowledge graph construction.
...
```bash
helm install ies-ontology-producer .
```
...
| `image.repository`  | Container image name                                                     | `telicent/tc-ies-ontology-producer` |
