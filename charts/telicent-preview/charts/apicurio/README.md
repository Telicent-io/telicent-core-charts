# Telicent Package for Apicurio Registry

Apicurio Registry provides secure and persistent storage for the JSON schemas used by the Notifications Framework when
notification validation is enabled.

## Introduction

This chart bootstraps an Apicurio Registry deployment on a [Kubernetes](https://kubernetes.io) cluster using
...
```console
helm install my-release ./charts/telicent-preview/charts/apicurio
```
...
.dev/readme-generator-for-helm --config=charts/telicent-core/readme.config \
 --values=charts/telicent-preview/charts/apicurio/values.yaml \
 --readme=charts/telicent-preview/charts/apicurio/README.md \
 --schema=charts/telicent-preview/charts/apicurio/values.schema.json
```
...
| Name                   | Description                                                     | Value |
| ---------------------- | --------------------------------------------------------------- | ----- |
| `replicas`             | Number of *Apicurio* replicas to deploy                         | `1`   |
| `revisionHistoryLimit` | Number of controller revisions to keep                          | `5`   |
| `annotations`          | Add extra annotations to the deployment object                  | `{}`  |
| `podLabels`            | Add extra labels to the *Apicurio* pod                          | `{}`  |
| `podAnnotations`       | Add extra annotations to the *Apicurio* pod                     | `{}`  |
| `extraEnvVars`         | Array with extra environment variables to add to *Apicurio* pod | `[]`  |
...
| Name                | Description                                                               | Value                        |
| ------------------- | ------------------------------------------------------------------------- | ---------------------------- |
| `image.registry`    | *Apicurio* image registry                                                 | `docker.io`                  |
| `image.repository`  | *Apicurio* image name                                                     | `apicurio/apicurio-registry` |
| `image.tag`         | *Apicurio* image tag. If not set, a tag is generated using the appVersion | `""`                         |
| `image.pullPolicy`  | *Apicurio* image pull policy                                              | `IfNotPresent`               |
...
| Name        | Description                         | Value |
| ----------- | ----------------------------------- | ----- |
| `resources` | Resources for *Apicurio* containers | `{}`  |
...
| Name           | Description                                                                 | Value       |
| -------------- | --------------------------------------------------------------------------- | ----------- |
| `service.name` | *Apicurio* service name. If not set, a name is generated using the fullname | `""`        |
| `service.port` | *Apicurio* service port                                                     | `8080`      |
| `service.type` | *Apicurio* service type                                                     | `ClusterIP` |
...
The following contains connection details to a PostgreSQL instance used to provide persistent storage of JSON schemas.
It is recommended to store sensitive information including passwords in a Kubernetes secret and not in Helm values.
For Quick Start purposes, a secret named `tc-auth-usr-psql-apicurio` will be created if one is not set.
...
*Apicurio* interacts with applications deployed via *telicent-core* using their default service/serviceAccount and port.
