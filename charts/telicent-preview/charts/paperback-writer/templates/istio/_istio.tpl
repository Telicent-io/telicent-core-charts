...
b.) when installed through a sub chart, where release name equals 'paperback-writer' (chart name) the
    release name will not be included.
*/}}
{{- define "paperback-writer.calServiceAccount" -}}
{{- $envVal := index . 0 -}}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "paperback-writer.calServiceAccount" (list . .Values.istio.ingress.serviceAccountName )) }}
{{- end }}
{{- end }}
