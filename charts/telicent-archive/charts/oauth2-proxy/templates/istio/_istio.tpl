...
b.) when installed through a sub chart, where release name equals 'oauth2-proxy' (chart name) the
    release name will not be included.
*/}}
{{- define "oauth2-proxy.calServiceAccount" -}}
{{- $envVal := index . 0 -}}
...
{{- else }}
{{- printf "- cluster.local/ns/%s/sa/%s" .Release.Namespace (include "oauth2-proxy.calServiceAccount" (list . .Values.istio.ingress.serviceAccountName) ) }}
{{- end }}
{{- end }}
